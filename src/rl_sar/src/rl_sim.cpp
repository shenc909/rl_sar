...
// Rasterize the self-filtered lidar cloud into the 2-channel BEV height_scan the policy was trained on
// (mirrors AccumulatedLidarBEV._emit): a grid_h x grid_w grid in the yaw-aligned base frame, channel-major
...
std::vector<float> RL_Sim::ComputeHeightScanBEV()
{
    ...
    {
        return this->MakeHeightScanFill(); // no scan yet -> flat-ground fallback
    }

    const int num_points = this->params.Get<int>("num_height_scan_points");
    const int num_channels = std::max(1, this->params.Get<int>("height_scan_num_channels", 2));
    const auto x_range = this->params.Get<std::vector<float>>("bev_x_range");
    const auto y_range = this->params.Get<std::vector<float>>("bev_y_range");
    const float res = this->params.Get<float>("bev_resolution");
    const std::string height_stat = this->params.Get<std::string>("bev_height_stat");
    const auto mount_xyz = this->params.Get<std::vector<float>>("bev_lidar_mount_xyz");
    const auto mount_rpy = this->params.Get<std::vector<float>>("bev_lidar_mount_rpy");
    ...
    if (cells != per_channel)
    {
        ...
        return this->MakeHeightScanFill();
    }
    ...
    const std::vector<float> q_base = this->obs.base_quat;          // [w,x,y,z]
